...
use crate::g::{fs::{FsError, Path}, subprocess::{Subprocess, SubprocessFn}, Game};
...
pub const CMD: Subprocess = {
	...
	impl<'a> Completer for RlHelper<'a> {
		...
		) -> rustyline::Result<(usize, Vec<Self::Candidate>)>
		{
			...
			else if let Some(space_index) = Self::rfind_unescaped_if(&line[..pos], char::is_whitespace) {
				let word = if let Some(end_space) = Self::find_unescaped_if(&line[pos..], char::is_whitespace) {
			...
			// if this is the first word
				...
				let word = if let Some(space_idx) = Self::find_unescaped_if(&line[pos..], char::is_whitespace) {
					&line[..space_idx]
				} else {
					line
				};
				let completions = self.0.current_computer().exes().iter()
	...
	impl SubprocessFn for Cmd {
		fn run(&self, g: &Game, _args: Vec<String>) -> std::io::Result<()> {
			...
			while !g.current_computer().should_quit.get() {
				let line = {
					let ps1 = g.current_computer().env.borrow()
						.get(&"PS1".to_string())
						.cloned()
